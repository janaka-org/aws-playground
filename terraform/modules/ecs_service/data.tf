...
data "template_file" "continer_definitions" {
  ...
  template = file("${path.module}/container-definitions_envoy:otel:cwagent.json.tpl")

  # keep render-taskdef.tf in sync with changes here.
  vars = {
    container_name              = var.app_name
    image_url                   = "${local.aws_account_id}.dkr.ecr.${local.aws_region}.amazonaws.com/${var.app_container_image}" # pulling the apps images from a single region
